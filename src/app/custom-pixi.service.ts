...
export class CustomPixiService {
  ...
  public changeBackground(room: Room) {
    ...
    if (!!!room.backgroundeffect) {
      var background = PIXI.Sprite.fromImage(
        "https://nekotobias-ef.glitch.me/" + room.background
      );
      ...
      this.pApp.ticker.add(() => {
        ...
        if (dragging) {
          ...
          // graphics.endFill();
          this.socketService.sendDrawCoordinates(
            newPosition.x,
            newPosition.y,
            1
          );
      ...
      let oldx;
      let oldy;
      this.socketService.onResetDrawCoordinates().subscribe((ch: any) => {
        oldx = null;
        oldy = null;
      });
      this.socketService.onSendDrawCoordinates().subscribe((ch: any) => {
        if (!!!oldx || !!!oldy) {
          oldx = ch[0];
          oldy = ch[1];
        }
  ...
  public drawUser(chatter: Chatter): void {
    ...
    let sprite = PIXI.Sprite.fromImage(
      "https://nekotobias-ef.glitch.me/" + chatter.currentavatar
    );
    ...
    this.socketService.onSendPosition().subscribe((ch: any) => {
    ...
    this.socketService.onInitPlayerPos().subscribe((ch: any) => {
      if (ch[0].id !== chatter.id) {
    ...
    map[chatter.id] = this.pApp.stage.addChild(sprite);
    ...
    this.socketService.onMoveUser().subscribe((chxy: any) => {
    ...
    this.pApp.ticker.add(() => {
      ...
      if (!!mx && !!my) {
        ...
        // mousePosition=getMousePosition();
        if (
          Math.abs(sprite.x - mx) >= epsilon &&
          Math.abs(sprite.y - my) >= epsilon
        ) {
          let dx = mx - sprite.x;
          let dy = my - sprite.y;
